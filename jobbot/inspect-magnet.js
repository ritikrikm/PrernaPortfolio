...
(async () => {
  ...
  console.log('STEP1_BUTTONS=' + JSON.stringify((await page.getByRole('button').allTextContents()).map(x=>x.trim()).filter(Boolean)));
  ...
  console.log('AFTER_MANUAL_BODY=' + (await page.locator('body').innerText()).replace(/\s+/g,' ').slice(0,2500));
